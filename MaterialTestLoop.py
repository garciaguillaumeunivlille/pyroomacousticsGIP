...
matMap = {
    "Wood_Parquet": [0.18, 0.12, 0.1, 0.09, 0.08, 0.07, 0.07, 0.07],
    "Wood_Walls": [0.15, 0.11, 0.10, 0.07, 0.06, 0.07, 0.07, 0.07],
    "Wood_Deco": [0.10, 0.07, 0.05, 0.04, 0.04, 0.04, 0.04, 0.04],
    "Limestone": [0.02, 0.02, 0.03, 0.04, 0.05, 0.05, 0.05, 0.05],
    "Plaster ": [0.01, 0.02, 0.02, 0.03, 0.04, 0.05, 0.05, 0.05],
    "Fibre_Lin ": [0.07, 0.07, 0.2, 0.41, 0.75, 0.97, 0.97, 0.97],
}

for kMatName, vCoeff in matMap.items():
    ...
    simpleMeshMatMap = {
        "Theatre_Wood_Parquet": {
            "stlFileName": "Theatre_Wood_Parquet.stl",
            "material": pra.Material(energy_absorption=0.01, scattering=0.0),
        },
        "Theatre_Wood_Walls": {
            "stlFileName": "Theatre_Wood_Walls.stl",
            "material": pra.Material(energy_absorption=0.01, scattering=0.0),
    ...
    for k, v in meshMatMap.items():
        ...
        the_mesh = mesh.Mesh.from_file(
            Path(f"PyroomMeshes/ReworkedMeshes/{stlFileName}")
        )
    ...
    if len(computedIRs) == len(room.mic_array):
        ...
        for i in range(0, len(room.mic_array)):
            ...
            signal = exportIRToWav(
                ...
                norm=False,
                fileName=f"{RenderARGS["exportPath"]}/{wavFileName}",
                micIndex=(i),
            )
